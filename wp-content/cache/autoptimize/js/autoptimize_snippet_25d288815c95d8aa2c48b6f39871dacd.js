...
MYMAP.init('#wpgmza_map',myLatLng,parseInt(wpgmaps_localize[wpgmaps_mapid].map_start_zoom));UniqueCode=Math.round(Math.random()*10000);MYMAP.placeMarkers(wpgmaps_markerurl+'?u='+UniqueCode,wpgmaps_localize[wpgmaps_mapid].id,null,null,null);if(wpgmaps_localize[wpgmaps_mapid].other_settings.store_locator_style=='modern')
{if(!MYMAP.map)
...
return;if(WPGMZA.settings.engine!="google-maps")
...
MYMAP.init=function(selector,latLng,zoom){var maptype=null;if(window.google)
...
return;element.setAttribute("data-map-id",1);element.setAttribute("data-maps-engine",WPGMZA.settings.engine);this.map=WPGMZA.Map.createInstance(element,myOptions);this.bounds=new WPGMZA.LatLngBounds();if(MYMAP.modernStoreLocator)
...
{var style=wpgmaps_localize[map_id].other_settings.wpgmza_store_locator_radius_style;if(WPGMZA.isDeviceiOS())
style="legacy";switch(style)
{case"modern":if(MYMAP.modernStoreLocatorCircle)
MYMAP.modernStoreLocatorCircle.destroy();MYMAP.modernStoreLocatorCircle=WPGMZA.ModernStoreLocatorCircle.createInstance(map_id);MYMAP.modernStoreLocatorCircle.setOptions({visible:true,center:center,radius:radius*(distance_type==1?WPGMZA.Distance.KILOMETERS_PER_MILE:1),radiusString:radius});break;default:var options={strokeColor:'#FF0000',strokeOpacity:0.25,strokeWeight:2,fillColor:'#FF0000',fillOpacity:0.15,map:MYMAP.map,center:center};if(distance_type=="1")
options.radius=parseInt(radius/0.000621371);else
options.radius=parseInt(radius/0.001);if(typeof wpgmza_last_default_circle!=="undefined"&&wpgmza_last_default_circle!==null){wpgmza_last_default_circle.setMap(null);wpgmza_last_default_circle=null;}
wpgmza_last_default_circle=WPGMZA.Circle.createInstance(options);break;}}
...
temp_actiontype="click";marker.on(temp_actiontype,function(){if(window.infoWindow)
infoWindow.close();if(!wpgmaps_localize_global_settings["wpgmza_settings_disable_infowindows"])
{if(!window.infoWindow)
wpgmza_init_infowindow();infoWindow.setContent(html);infoWindow.open(MYMAP.map,marker);}});marker_array[wpmgza_marker_id]=marker;}}});});}else{if(Object.keys(wpgmaps_localize_marker_data).length>0){var markerStoreLocatorsNum=0;if(typeof wpgmaps_localize_marker_data!=="undefined"){jQuery.each(wpgmaps_localize_marker_data,function(i,val){var wpmgza_map_id=val.map_id;if(wpmgza_map_id==map_id){var wpmgza_address=val.address;var wpmgza_anim=val.anim;var wpmgza_infoopen=val.infoopen;var lat=val.lat;var lng=val.lng;var point=new WPGMZA.LatLng(lat,lng);var wpmgza_marker_id=val.marker_id;var current_lat=val.lat;var current_lng=val.lng;var show_marker_radius=true;if(radius!==null){if(check1>0){}else{var point=searched_center;if(typeof wpgmaps_localize[wpgmaps_mapid]['other_settings']['store_locator_bounce']==="undefined"||wpgmaps_localize[wpgmaps_mapid]['other_settings']['store_locator_bounce']===1){var marker=WPGMZA.Marker.createInstance({position:point,map:MYMAP.map});marker_sl=marker;}else{}
...
marker_sl=marker;}
wpgmza_show_store_locator_radius(map_id,point,radius,distance_type);check1=check1+1;}}
function add_polygon(polygonid){if(WPGMZA.settings.engine!="google-maps")
...
function add_polyline(polyline){if(WPGMZA.settings.engine!="google-maps")
...
MYMAP.placeMarkers(wpgmaps_markerurl+'?u='+UniqueCode,wpgmaps_localize[wpgmaps_mapid].id,radius,center_searched,distance_type);var event={type:"storelocatorresult",position:center_searched};MYMAP.map.trigger(event);}
